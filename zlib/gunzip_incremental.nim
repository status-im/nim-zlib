...
## Incremental gunzip based on `Stream` input.
...
proc init*(
  ...
  let chunkLen = inStream.readData(addr chunk, chunk.len)
...
iterator line*(gz: GUnzipRef): string {.gcsafe, raises: [IOError, OSError].} =
